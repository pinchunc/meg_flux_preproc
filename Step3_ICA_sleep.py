...
for subject in sub_list:
    ...
    #l_freq = 1
    #h_freq = 90 # for gamma
    resample_sfreq = 200 # this is just for ICA not actually change the data
    ...
    task_files = [file for file in files if file.endswith('ann.fif') & file.startswith(f'sub-{subject}_ses-{session}_task-{task}_r')]
    ...
    for run_idx in range(len(task_files)):
        ...
        ica.fit(raw_resmpl.pick(['meg','eog','ecg','emg']),verbose=True) #'emg' for sleep
