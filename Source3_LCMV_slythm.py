...
deriv_file_nrem = deriv_file_nrem.replace('epo', '1sec_epo') 
deriv_fname_nrem = op.join(bids_path.directory, deriv_file_nrem)

fwd_fname = bids_path.basename.replace(epo_suffix, fwd_suffix)
fwd_fname = fwd_fname('sleep', 'loc') # temporary, later will need to create on for sleep
...
epochs_stage3  = epochs[['stage0']].copy()
epochs_stage2  = epochs[['stage2']].copy()
epochs_stage3  = epochs[['stage3']].copy()
epochs_stage5  = epochs[['stagerem']].copy()

#%% Calculating the covariance matrices 
# only use the epochs of interests
