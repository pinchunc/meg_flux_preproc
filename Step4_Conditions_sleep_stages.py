...
sub_list = [116]#['106', '107', '112', '118', '110', '111', '116', '117', '119']#'102', '103', '104', 
# 
...
for subject in sub_list:
    ...
    for run_idx in range(len(task_files)):
        ...
        if new_onsets[-1] < total_duration:
            new_durations.append(total_duration-new_onsets[-1]-1)
        ...
        #             task=task, run=run, suffix=epo_suffix, datatype='meg',
        #             root=deriv_root, extension='.fif', check=False)
        # deriv_file_nrem_1sec = bids_path.basename.replace('task-sleep', 'task-sleep-staged')
        # deriv_file_nrem_1sec = deriv_file_nrem_1sec.replace('epo', '1sec_epo') 
        # deriv_fname_nrem_1sec = op.join(bids_path.directory, deriv_file_nrem_1sec)
        # epochs.save(deriv_fname_nrem_1sec, overwrite=True)
